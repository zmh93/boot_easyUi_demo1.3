...
(function($){
	...
	function setSize(target, param){
		...
		if (t.find('.calendar-menu.json').is(':visible')){
	...
	function init(target){
		$(target).addClass('calendar').html(
				...
				'<div class="calendar-body">' +
					'<div class="calendar-menu.json">' +
						'<div class="calendar-menu.json-year-inner">' +
							'<span class="calendar-nav calendar-menu.json-prev"></span>' +
							'<span><input class="calendar-menu.json-year" type="text"></input></span>' +
							'<span class="calendar-nav calendar-menu.json-next"></span>' +
						'</div>' +
						'<div class="calendar-menu.json-month-inner">' +
						'</div>' +
					'</div>' +
				'</div>'
		);
	...
	function bindEvents(target){
		var opts = $.data(target, 'calendar').options;
		var menu = $(target).find('.calendar-menu.json');
		menu.find('.calendar-menu.json-year').unbind('.calendar').bind('keypress.calendar', function(e){
			if (e.keyCode == 13){
				setDate(true);
			}
		});
		$(target).unbind('.calendar').bind('mouseover.calendar', function(e){
			...
			var t = toTarget(e.target);
			if (t.hasClass('calendar-menu.json-next') || t.hasClass('calendar-nextyear')){
				showYear(1);
			} else if (t.hasClass('calendar-menu.json-prev') || t.hasClass('calendar-prevyear')){
				showYear(-1);
			} else if (t.hasClass('calendar-menu.json-month')){
		...
		function setDate(hideMenu){
			var menu = $(target).find('.calendar-menu.json');
			var year = menu.find('.calendar-menu.json-year').val();
		...
		function showYear(delta){
			opts.year += delta;
			show(target);
			menu.find('.calendar-menu.json-year').val(opts.year);
		}
	...
	/**
	 * show the select menu.json that can change year or month, if the menu.json is not be created then create it.
	 */
	function showSelectMenus(target){
		var opts = $.data(target, 'calendar').options;
		$(target).find('.calendar-menu.json').show();
		
		if ($(target).find('.calendar-menu.json-month-inner').is(':empty')){
			$(target).find('.calendar-menu.json-month-inner').empty();
			var t = $('<table class="calendar-mtable"></table>').appendTo($(target).find('.calendar-menu.json-month-inner'));
			var idx = 0;
			for(var i=0; i<3; i++){
				var tr = $('<tr></tr>').appendTo(t);
				for(var j=0; j<4; j++){
					$('<td class="calendar-nav calendar-menu.json-month"></td>').html(opts.months[idx++]).attr('abbr',idx).appendTo(tr);
				}
			}
		}
		
		var body = $(target).find('.calendar-body');
		var sele = $(target).find('.calendar-menu.json');
		var seleYear = sele.find('.calendar-menu.json-year-inner');
		var seleMonth = sele.find('.calendar-menu.json-month-inner');
	...
	$.fn.calendar = function(options, param){
		...
		return this.each(function(){
			...
			bindEvents(this);
			show(this);
			$(this).find('div.calendar-menu.json').hide();	// hide the calendar menu.json
		});
	};
