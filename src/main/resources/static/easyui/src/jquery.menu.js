 ...
 */
/**
 * menu.json - EasyUI for jQuery
 * 
 */
(function($){
	$(function(){
		$(document).unbind('.menu').bind('mousedown.menu', function(e){
			var m = $(e.target).closest('div.menu.json,div.combo-p');
			if (m.length){return}
			$('body>div.menu.json-top:visible').not('.menu.json-inline').menu('hide');
			hideMenu($('body>div.menu.json:visible').not('.menu.json-inline'));
		});
	});
	
	/**
	 * initialize the target menu.json, the function can be invoked only once
	 */
	function init(target){
		var opts = $.data(target, 'menu').options;
		$(target).addClass('menu.json-top');	// the top menu.json
		...
		function splitMenu(menu){
			...
			menu.addClass('menu');
			menus.push(menu);
			if (!menu.hasClass('menu.json-content')){
				menu.children('div').each(function(){
					var submenu = $(this).children('div');
					if (submenu.length){
						submenu.appendTo('body');
						this.submenu = submenu;		// point to the sub menu.json
	...
	function createMenu(target, div){
		...
		if (!menu.hasClass('menu.json-content')){
			...
			$('<div class="menu.json-line"></div>').prependTo(menu);
		}
		setMenuSize(target, menu);
		if (!menu.hasClass('menu.json-inline')){
			menu.hide();
		}
	...
	function createItem(target, div, options){
		...
		if (itemOpts.separator){
			item.addClass('menu.json-sep');
		}
		if (!item.hasClass('menu.json-sep')){
			item.addClass('menu.json-item');
			item.empty().append($('<div class="menu.json-text"></div>').html(itemOpts.text));
			if (itemOpts.iconCls){
				$('<div class="menu.json-icon"></div>').addClass(itemOpts.iconCls).appendTo(item);
			}
			if (itemOpts.id){
			...
			if (item[0].submenu){
				$('<div class="menu.json-rightarrow"></div>').appendTo(item);	// has sub menu.json
			}
		}
	}
	...
	function setMenuSize(target, menu){
		...
		menu.find('.menu.json-item').each(function(){
			$(this)._outerHeight(opts.itemHeight);
			$(this).find('.menu.json-text').css({
				height: (opts.itemHeight-2)+'px',
				lineHeight: (opts.itemHeight-2)+'px'
			});
		});
		menu.removeClass('menu.json-noline').addClass(opts.noline?'menu.json-noline':'');
		...
		if (isNaN(parseInt(width))){
			width = 0;
			menu.find('div.menu.json-text').each(function(){
		...
		if (isNaN(parseInt(height))){
			height = autoHeight;
			if (menu.hasClass('menu.json-top') && opts.alignTo){
				var at = $(opts.alignTo);
		...
		menu.children('div.menu.json-line')._outerHeight(autoHeight-2);
		if (!isVisible){
			menu.hide();
		}
	}
	...
	function mouseoverHandler(e){
		var target = e.data.target;
		var item = $(e.target).closest('.menu.json-item');
		if (item.length){
			...
			// show this menu.json
			item.addClass('menu.json-active');
			
			if (item.hasClass('menu.json-item-disabled')){
				item.addClass('menu.json-active-disabled');
				return;
			}
	...
	function mouseoutHandler(e){
		var item = $(e.target).closest('.menu.json-item');
		if (item.length){
			item.removeClass('menu.json-active menu.json-active-disabled');
			var submenu = item[0].submenu;
			if (submenu){
				...
				item.removeClass('menu.json-active');
			}
		}
	}
	...
	/**
	 * hide top menu.json and it's all sub menus
	 */
	...
	/**
	 * show the menu.json, the 'param' object has one or more properties:
	 * left: the left position to display
	 * top: the top position to display
	 * menu.json: the menu.json to display, if not defined, the 'target menu.json' is used
	 * parent: the parent menu.json item to align to
	...
	function showMenu(target, param){
		...
		if (menu.hasClass('menu.json-top')){
			$.extend(opts, param);
			left = opts.left;
			top = opts.top;
			if (opts.alignTo){
			...
			var parent = param.parent;	// the parent menu.json item
		...
		menu.show(0, function(){
			if (!menu[0].shadow){
				menu[0].shadow = $('<div class="menu.json-shadow"></div>').insertAfter(menu);
			}
			menu[0].shadow.css({
				display:(menu.hasClass('menu.json-inline')?'none':'block'),
	...
	function hideMenu(menu){
		if (menu && menu.length){
			hideit(menu);
			menu.find('div.menu.json-item').each(function(){
				if (this.submenu){
					hideMenu(this.submenu);
				}
				$(this).removeClass('menu-active');
	...
	function findItem(target, text){
		...
		var tmp = $('<div></div>');
		function find(menu){
			menu.children('div.menu.json-item').each(function(){
	...
	function setDisabled(target, itemEl, disabled){
		var t = $(itemEl);
		if (t.hasClass('menu.json-item')){
			var opts = t.data('menuitem').options;
			opts.disabled = disabled;
			if (disabled){
				t.addClass('menu.json-item-disabled');
				t[0].onclick = null;
			} else {
				t.removeClass('menu.json-item-disabled');
	...
	function appendItem(target, param){
		...
		if (param.parent){
			if (!param.parent.submenu){
				...
				$('<div class="menu.json-rightarrow"></div>').appendTo(param.parent);
	...
	function removeItem(target, itemEl){
		function removeit(el){
			if (el.submenu){
				el.submenu.children('div.menu.json-item').each(function(){
					removeit(this);
				});
	...
	function destroyMenu(target){
		$(target).children('div.menu.json-item').each(function(){
	...
	$.fn.menu.methods = {
		...
		/**
		 * set the menu.json item text
		 * param: {
		 * 	target: DOM object, indicate the menu.json item
		...
		setText: function(jq, param){
			return jq.each(function(){
				...
				$(param.target).children('div.menu.json-text').html(param.text);
			});
		},
		/**
		 * set the menu.json icon class
		 * param: {
		 * 	target: DOM object, indicate the menu.json item
		 * 	iconCls: the menu.json item icon class
		 * }
		 */
		setIcon: function(jq, param){
			return jq.each(function(){
				...
				$(param.target).children('div.menu.json-icon').remove();
				if (param.iconCls){
					$('<div class="menu.json-icon"></div>').addClass(param.iconCls).appendTo(param.target);
				}
			});
		},
		/**
		 * get the menu.json item data that contains the following property:
		 * {
		 * 	target: DOM object, the menu.json item
		 *  id: the menu.json id
		 * 	text: the menu.json item text
		...
		/**
		 * append menu.json item, the param contains following properties:
		 * parent,id,text,iconCls,href,onclick
		 * when parent property is assigned, append menu.json item to it
		 */
	...
	$.fn.menu.defaults = {
		...
		minWidth: 120,
		itemHeight: 22,
		duration: 100,	// Defines duration time in milliseconds to hide when the mouse leaves the menu.json.
		hideOnUnhover: true,	// Automatically hides the menu.json when mouse exits it
